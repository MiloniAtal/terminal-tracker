...
lint:  ## run static analysis with flake8
	python -m black --check terminal_tracker setup.py
	python -m flake8 terminal_tracker setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python -m black terminal_tracker/ setup.py
...
annotate:  ## run type checking
	python -m mypy ./terminal_tracker

#########
# TESTS #
#########
test: ## clean and run unit tests
	python -m pytest -v terminal_tracker/tests

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v terminal_tracker/tests --cov=terminal_tracker --cov-branch --cov-fail-under=75 --cov-report term-missing 
